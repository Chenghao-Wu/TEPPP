...
vector<double> unwrap_rec(vector<double> p1, vector<double> p2, double box_dim_x, double box_dim_y, double box_dim_z)
{
	int imgx = round((p1[0] - p2[0]) / box_dim_x);
	int imgy = round((p1[1] - p2[1]) / box_dim_y);
	int imgz = round((p1[2] - p2[2]) / box_dim_z);

	return {p2[0] + (imgx * box_dim_x), p2[1] + (imgy * box_dim_y), p2[2] + (imgz * box_dim_z)};
}
 ...
 * @return A 2d array containing the unwrapped coordinates for each atom in the system
 */
double** read_coords_rec(string fname, int* n, int chain_length, double box_dim_x, double box_dim_y, double box_dim_z)
{
	string line;
	ifstream myfile;
	myfile.open(fname);
	...
	while (getline(myfile, line))
	{
		...
		if (count > 0)
		{
			...
			{
				temp.push_back(unwrap_rec(
					temp[temp.size() - 1],
					{stod(tokens[0], &sz), stod(tokens[1], &sz), stod(tokens[2], &sz)},
					box_dim_x,box_dim_y,box_dim_z));
			}
		}
	}
